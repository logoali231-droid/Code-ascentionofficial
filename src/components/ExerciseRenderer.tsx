...
interface Exercise {
  id?: string;
  type?: "mcq" | "text" | "code" | "output" | "project";
  language?: string;
  question: string;
  options?: string[];
  answer?: string;
...
interface ExerciseProps {
  ...
  onNext?: (success: boolean) => Promise<void> | void;
}
...
export default function ExerciseRenderer({
  ...
  // Ajuste dinâmico para teclado mobile (Samsung M23)
  useEffect(() => {
    const handleResize = () => {
      if (window.visualViewport) {
        setAvailableHeight(`${window.visualViewport.height}px`);
      }
    };
  ...
  return (
    <div 
      style={{ height: `calc(${availableHeight} - 2rem)` }}
      className={`flex flex-col rounded-2xl border-2 bg-slate-900/90 transition-all duration-300 overflow-hidden ${
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-5 relative custom-scrollbar">
        <h2 className="text-xl font-bold mb-4 text-slate-100 italic uppercase leading-tight">
          {exercise.question}
        </h2>
        ...
        {exercise.type === "code" ? (
          <div className="space-y-4">
            <CodeEditor
              language={exercise.language || "plaintext"}
          ...
          <div className="space-y-3">
            {exercise.options?.map((opt, i) => (
              <button
                key={i}
                onClick={() => { setSelected(opt); handleValidation(opt); }}
        ...
        {/* Espaçador para o Fade-out não cobrir o texto final */}
        <div className="h-20" />
      </div>

      {/* FOOTER / BOTÃO DE VALIDAÇÃO FIXO */}
      <div className="p-4 bg-slate-900 border-t border-slate-800">
        {exercise.type === "code" && (
          <button
            onClick={() => handleValidation(codeAnswer)}
            disabled={status === "checking" || status === "success"}
            className="w-full py-4 rounded-xl bg-cyan-600 text-white font-bold uppercase tracking-widest hover:bg-cyan-500 transition-all disabled:opacity-50"
          >
            {status === "checking" ? "Checking_Logic..." : "Validate_Neural_Pattern"}
          </button>
        )}
        
        {streamTotal > 0 && streamIndex < streamTotal && status !== "success" && (
          <div className="flex items-center justify-center gap-2 text-slate-600 mt-2">
            <Loader2 size={12} className="animate-spin" />
            <span className="text-[9px] uppercase tracking-tighter">Synchronizing_Next_Node...</span>
          </div>
        )}
      </div>

      {/* Efeito Fade-out de fundo */}
      <div className="absolute bottom-[80px] left-0 w-full h-12 pointer-events-none bg-gradient-to-t from-slate-900 to-transparent z-10" />
    </div>
  );
}