...
// =========================================================
// 🌍 ECOLOGICAL CONSTANTS (refined)
// =========================================================

// Brazil grid intensity (kgCO2/kWh)
const GRID_CO2_INTENSITY = 0.08;

// water usage split (important correction)
const CLOUD_WATER_PER_KWH_L = 1.8;
const LOCAL_WATER_PER_KWH_L = 0.05;

// mobile baseline idle consumption
const BASE_IDLE_WATTS = 0.8;

// network cost
const NETWORK_KWH_PER_GB = 0.06;

// cloud request footprint (lower, more realistic)
const CLOUD_CO2_PER_REQUEST = 0.0003;
const CLOUD_KWH_PER_REQUEST = 0.00012;

// tree absorption reference
const TREE_CO2_OFFSET_KG = 21;

// inference overhead boosts (adjusted downward)
const CPU_INFERENCE_BOOST = 0.8;
const GPU_INFERENCE_BOOST = 2.2;
...
// =========================================================
// DEVICE POWER HEURISTIC (overhead-only model)
// =========================================================

function estimateInferenceOverheadWatts(modelKey: string) {
  const cores = navigator.hardwareConcurrency || 4;
  const mem = (navigator as any).deviceMemory || 4;
  const hasWebGPU = "gpu" in navigator;

  const modelPower = MODEL_POWER_MAP[modelKey] || MODEL_POWER_MAP.default;

  // 🔧 IMPORTANT FIX:
  // now represents ONLY overhead above baseline device consumption
  let overhead =
    modelPower * 0.6 +
    cores * 0.04 +
    mem * 0.06;

  overhead += hasWebGPU ? GPU_INFERENCE_BOOST : CPU_INFERENCE_BOOST;
...
export default function ClientBody({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadUser() {
      ...
      if (!user) return router.push("/machineLock");
  ...
  useEffect(() => {
    ...
    window.fetch = async (...args) => {
      ...
      try {
        ...
        if (isModelAsset && !countedDownloads.has(url)) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const inferenceWatts = estimateInferenceOverheadWatts(model);

      if (inferenceActiveRef.current) {
        totalInferenceSecondsRef.current += deltaSeconds;

        const deltaKWh =
          (inferenceWatts * deltaSeconds) / 3600000;

        totalInferenceEnergyRef.current += deltaKWh;
      }

      const totalHours =
        (Date.now() - sessionStartRef.current) / 3_600_000;

      const idleKWh = (BASE_IDLE_WATTS * totalHours) / 1000;

      const networkKWh =
        (totalDownloadedMBRef.current / 1024) * NETWORK_KWH_PER_GB;

      const totalEnergy =
        idleKWh + totalInferenceEnergyRef.current + networkKWh;

      const totalCO2 = totalEnergy * GRID_CO2_INTENSITY;

      const totalWater =
        (idleKWh * LOCAL_WATER_PER_KWH_L) +
        ((totalEnergy - idleKWh) * CLOUD_WATER_PER_KWH_L);

      const treesEquivalent = totalCO2 / TREE_CO2_OFFSET_KG;

      setEnergy(totalEnergy);
      setCo2(totalCO2);
      setWater(totalWater);
      setTrees(treesEquivalent);
      setHours(totalHours);

      setCloudAvoided(cloudRequestsAvoidedRef.current);
      setCloudCO2Avoided(cloudCO2AvoidedKgRef.current);
      setCloudEnergyAvoided(cloudEnergyAvoidedRef.current);
  ...
  return (
    <>
      ...
      <div className="fixed bottom-3 right-3 z-50 bg-black/65 backdrop-blur-md border border-green-500/20 rounded-2xl px-4 py-3 text-[11px] text-green-300 shadow-xl flex flex-col gap-1 min-w-[210px]">
        <div>🌱 {trees.toFixed(6)} tree eq.</div>
        <div>⚡ {energy.toFixed(5)} kWh</div>
        <div>🌍 {co2.toFixed(6)} kgCO₂</div>
        <div>💧 {water.toFixed(4)} L</div>
        <div>🕒 {hours.toFixed(2)} h</div>

        <div className="opacity-70">
          ☁️ avoided {cloudAvoided.toFixed(0)} req
        </div>
      </div>
    </>
  );
