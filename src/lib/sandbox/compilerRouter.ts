...
import { db, getUser, get } from "../others/db";
import {
  UserStats,
  MemoryLog,
  CognitiveProfile,
...
import { calculateLevel } from "../others/level";


// 🌟 CORREÇÃO: Adicionado os campos de estratégia de roteamento adaptativo na interface de retorno
...
export async function getAdaptiveMetrics(
  ...
  // Cálculo de forma centralizada e amortecida sem dupla taxação externa
  ...
  // 🧠 Estratégia de Roteamento Baseada em Carga Cognitiva e Complexidade
  ...
  if (isADHD) {
    ...
    recommendedStrategy = 'HIGH_COMPUTATION_CLOUD'; // Algoritmos complexos exigem o cluster remoto
  }
  ...
  return {
    ...
    // Agora o TypeScript reconhece estas propriedades perfeitamente vindo de RewardMultiplier! ✅
