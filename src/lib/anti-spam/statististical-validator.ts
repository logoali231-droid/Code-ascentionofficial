export class StatisticalValidator {
  // Convenção: Primeira letra maiúscula para classes
  ...
  public filterTechnicalTerms(words: string[]): string[] {
    return words.filter(
      (word) => !this.TECH_EXCEPTIONS.has(word.toLowerCase()),
    );
  }
  /**
  ...
  public getStatisticalScore(text: string): number {
    ...
    const unlikelyBigrams =
      /q[^ueaion]|j[qxkz]|g[qj]|v[qj]|w[qj]|x[qzj]|z[qj]|[^aeiouy]{5,}/g;
