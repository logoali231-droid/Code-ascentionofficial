...
import { detectSystemCapabilities, AVAILABLE_MODELS } from "./modelManager"; 
...
export async function initEngine(modelId?: string, onProgress?: (p: any) => void) {
  ...
  try {
    ...
    // O erro 'find' acontece porque o web-llm não encontra a model_list 
    // ao passarmos um appConfig customizado.
    const config: webllm.ChatConfig = {
      kvCacheConfig: {
    ...
    const appConfig: webllm.AppConfig = {
      model_list: AVAILABLE_MODELS as any, // Força a lista de modelos conhecidos
      requiredCapabilities: {
        maxStorageBufferBindingSize: 419430400, // 400MB para o M23
      }
    };
    ...
    // Captura detalhada para o console mobile
...
export async function generate(
  ...
  try {
    ...
    if (String(err).includes("OutOfMemory") || String(err).includes("Device")) {
      if (!recovering) {
        recovering = true;
        await unloadEngine();
        await sleep(1000);
