...
self.onmessage = async (e: MessageEvent) => {
  ...
  if (type === "ABORT") {
    console.log("[Logic Worker] Comando de aborto recebido. Interrompendo pipeline de testes...");
    isAborted = true;
    return;
  }
  ...
  if (type === "EVALUATE_EXERCISE") {
    ...
    try {
      ...
      if (isAborted) throw new DOMException("Evaluation aborted.", "AbortError");
      ...
      if (isAborted) {
        console.warn("[Logic Worker] Resultado descartado: Usuário trocou de contexto.");
        return;
      }
