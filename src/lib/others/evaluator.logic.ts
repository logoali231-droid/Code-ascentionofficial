...
export async function evaluateLogic(
  ...
  try {
    ...
    let calculatedThreshold =
      typeof customThreshold === "number" && !isNaN(customThreshold)
        ? customThreshold
        : 0.72;
    try {
      ...
      const activeState = await get(
        "memory",
        `pedagogical_state_${currentCourseId.toLowerCase().trim()}`,
      );

      if (activeState) {
        if (
          activeState.struggleLevel > 0.6 ||
          activeState.cognitiveLoad > 0.7
        ) {
...
function tokenize(code: string) {
  return code.split(/[^a-z0-9_]+/gi).filter(Boolean);
}
