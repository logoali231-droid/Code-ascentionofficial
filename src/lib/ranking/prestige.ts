...
export class PrestigeManager {
   ...
   */
  static isEligibleForAscension(currentXp: number, currentLevel: number): boolean {
   ...
   */
  static performAscension(currentStats: PrestigeStats, currentXp: number, factionId: string) {
    ...
    const factionBonuses = FactionManager.getActiveBonuses(factionId, currentXp);
    const xpBoost = factionBonuses.find((b: FactionBonus) => b.type === 'XP_BOOST')?.value || 0;

    // Verifica novos Tiers desbloqueados
    const currentTiers = currentStats.unlockedTiers || [];
    const newlyUnlockedTiers = PRESTIGE_TIERS
      .filter(t => t.requirement <= newAscensionCount && !currentTiers.includes(t.tier))
      .map(t => t.tier);
    ...
    const newStats: PrestigeStats = {
      ...
      knowledgeMultiplier: currentStats.knowledgeMultiplier + (shardsGained * 0.05) + (xpBoost * 0.1),
      lastAscensionDate: Date.now(),
      unlockedTiers: [...currentTiers, ...newlyUnlockedTiers]
    };

    // Feedback baseado na facção
    const factionName = FactionManager.getFactionById(factionId)?.name || "Sistema";
  ...
  static getUpgradeCost(upgradeId: string, currentLevel: number): number {
    ...
    return upgrade ? Math.floor(upgrade.baseCost * Math.pow(1.5, currentLevel)) : Infinity;
  }
}