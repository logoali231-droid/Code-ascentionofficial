  ...
  return (
    <div className="p-4 pb-24 text-white min-h-screen bg-black font-sans">
      ...
      {showAscensionModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-100 flex items-center justify-center p-6">
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-md w-full">
            <h2 className="text-2xl font-bold text-red-500 mb-4 uppercase tracking-tighter">
              Aviso de Desintegração
            </h2>
            <p className="text-slate-300 mb-6 text-sm">
              A Ascensão resetará seu XP atual para Level 1 em troca de Shards
              Permanentes.
            </p>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-black italic flex gap-2 items-center">
          <Book size={20} className="text-blue-400" /> DATABASE_COURSES
        </h1>
        <div className="px-3 py-1 bg-slate-800 rounded-full text-[10px] font-bold text-cyan-400 border border-cyan-500/30">
          LVL_{userLevel}
        </div>
      </div>
        ...
        {courses.map((c: any) => (
          ...
          >
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-bold text-slate-100 group-hover:text-blue-400 transition-colors uppercase tracking-tight">
                  {c.topic}
                </h3>
                <p className="text-[10px] text-slate-500 font-mono mt-1">
                  DIFFICULTY: {c.difficulty} • NODES: {c.nodes || 12}
                </p>
              </div>
              <Zap
                size={14}
                className="text-slate-700 group-hover:text-yellow-500"
              />
            </div>
          </div>
        ))}
      ...
      <button
        ...
        className="mt-8 w-full bg-blue-600 hover:bg-blue-500 py-5 rounded-2xl font-black uppercase tracking-widest shadow-[0_10px_20px_rgba(37,99,235,0.2)]"
      >
        Novo Curso +
      </button>
    </div>
